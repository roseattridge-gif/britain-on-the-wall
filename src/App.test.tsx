...
describe('approved £100 national Wall',()=>{
  ...
  it('uses the approved stable source order',()=>{render(<App/>);const sources=screen.getAllByRole('button',{name:/percent$/}).slice(0,6);expect(sources[0]).toHaveAccessibleName(/Income & social contributions £45, 44.9 percent/);expect(sources[1]).toHaveAccessibleName(/Businesses £9.8/)});
  it('orders Health as the largest destination with sensible display precision',()=>{render(<App/>);const health=screen.getByRole('button',{name:/Health & social care £23, 23.1 percent/});expect(health).toHaveTextContent('£23');expect(health).toHaveTextContent('23.1%');expect(health).toHaveTextContent('£313.7bn')});
  it('keeps borrowing visibly separate from receipts',()=>{render(<App/>);const borrowing=screen.getByRole('button',{name:/Borrowing £9.5/});expect(borrowing).toHaveClass('borrowing');expect(borrowing).toHaveTextContent('Fills the gap · not revenue')});
  ...
  it('updates the same Wall from real historical adapters',async()=>{render(<App/>);await userEvent.click(screen.getByRole('button',{name:'2021–22'}));expect(screen.getByRole('button',{name:/Health & social care £24.6/})).toBeInTheDocument();expect(screen.getByText(/Select a later year/)).toBeInTheDocument()});
