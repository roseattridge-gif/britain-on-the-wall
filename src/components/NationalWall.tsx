...
function FiscalRow({item,total,unit,direction,rank,openEvidence}:{item:FiscalItem;total:number;unit:Unit;direction:'in'|'out';rank:number;openEvidence:(id:string)=>void}){
  const Icon=icons[item.id as keyof typeof icons]??Landmark;const share=item.value/total*100;const colour=item.borrowing?'#7253a0':item.technical?'#92999b':tones[direction==='in'?rank:rank+6];
  return <button style={{'--row-colour':colour} as React.CSSProperties} className={`fiscal-row ${direction} ${rank<3?'major':''} ${item.borrowing?'borrowing':''} ${item.technical?'technical':''}`} onClick={()=>openEvidence(item.evidenceId)} aria-label={`${item.name} ${money(item.value,total,unit)}, ${share.toFixed(1)} percent`}>
...
export function NationalWall({year,setYear,unit,setUnit,openEvidence}:Props){
  ...
  return <main className="national-wall">
    ...
    <section className="money-stage">
      <svg className="editorial-flows" viewBox="0 0 1400 620" preserveAspectRatio="none" aria-hidden="true">{sources.map((item,i)=><path key={item.id} className={item.borrowing?'borrow-flow':''} d={`M 310 ${88+i*81} C 470 ${88+i*81}, 505 ${310+(i-2.5)*18}, 638 310`} stroke={item.borrowing?'#7253a0':tones[i]} strokeWidth={Math.max(5,item.value/total*80)} />)}{destinations.map((item,i)=><path key={item.id} className={item.technical?'technical-flow':''} d={`M 762 310 C 875 ${310+(i-6)*12}, 920 ${48+i*40}, 1085 ${48+i*40}`} stroke={item.technical?'#92999b':tones[i+6]} strokeWidth={Math.max(4,item.value/total*72)} />)}</svg>
