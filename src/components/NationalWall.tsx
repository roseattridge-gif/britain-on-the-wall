...
export function NationalWall({year,setYear,unit,setUnit,openEvidence}:Props){
  ...
  return <main className="national-wall">
    ...
    <section className="money-stage">
      ...
      <section className="fiscal-column sources"><div className="section-heading"><small>01 · MONEY IN</small><h1>Where Britain’s £100 comes from</h1></div>{sources.map((item,i)=><FiscalRow key={item.id} item={item} total={total} unit={unit} direction="in" rank={i} openEvidence={openEvidence}/>)}</section>
      ...
      <section className="fiscal-column destinations"><div className="section-heading"><small>02 · MONEY OUT</small><h1>Where Britain’s £100 goes</h1></div>{destinations.map((item,i)=><FiscalRow key={item.id} item={item} total={total} unit={unit} direction="out" rank={i} openEvidence={openEvidence}/>)}</section>
    </section>
