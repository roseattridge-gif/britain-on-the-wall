...
export function NationalWall({year,setYear,unit,setUnit,openEvidence}:Props){
  const[showHelp,setShowHelp]=useState(false);const[story,setStory]=useState<StoryId>('immigration');const total=totalFunding(wallData,year);const yearIndex=realYears.indexOf(year as RealYear);const previousYear=realYears[Math.max(0,yearIndex-1)];const previousTotal=totalFunding(wallData,previousYear),baseTotal=totalFunding(wallData,2021);
  ...
  return <main className="national-wall">
    ...
    <section className="lower-editorial" id="history"><HowWeGotHere year={year} setYear={setYear} openEvidence={openEvidence} story={story} setStory={setStory}/></section><footer className="editorial-footer"><b>◇ &nbsp; TRANSPARENT. EVIDENCE-BASED. INDEPENDENT.</b><span>All data is from official sources. We show what the evidence says — not what we think.</span><nav>Sources &nbsp; • &nbsp; Methodology &nbsp; • &nbsp; Data dictionary ↗</nav></footer>
  </main>;
}
