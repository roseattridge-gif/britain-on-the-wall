  ...
  return <>
    <div className="canvas-tools" aria-label="Wall layers">
      {dataMode==='demo'?<><label className="timeline-control">
        ...
        <b>{year}</b>
      </label><div className="timeline-ticks" aria-label="Demo years">{years.map(item=><button key={item} aria-label={`Year ${item}`} className={year===item?'active':''} onClick={()=>setYear(item)}>{String(item).slice(2)}</button>)}</div></>:<><label className="timeline-control real-timeline"><span>OUTTURN</span><input aria-label="Fiscal period timeline" type="range" min="0" max="4" step="1" value={realYears.indexOf(year as RealYear)} onChange={e=>setYear(realYears[Number(e.target.value)])}/><b>{displayPeriod(year)}</b></label><div className="timeline-ticks real-ticks" aria-label="Available fiscal periods">{realYears.map(item=><button key={item} aria-label={`Select fiscal period ${displayPeriod(item)}`} aria-pressed={year===item} className={year===item?'active':''} onClick={()=>setYear(item)}>{displayPeriod(item).slice(2)}</button>)}</div><button className={playing?'active':''} aria-pressed={playing} onClick={()=>setPlaying(!playing)}>{playing?'PAUSE':'PLAY 5 YEARS'}</button><button className={showFiveYear?'active':''} aria-pressed={showFiveYear} onClick={()=>setShowFiveYear(!showFiveYear)}>5-YEAR CHANGE</button></>}
