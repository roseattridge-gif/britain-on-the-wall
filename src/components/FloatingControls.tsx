  ...
  return <>
    <div className="canvas-tools" aria-label="Wall layers">
      {dataMode==='demo'?<><label className="timeline-control">
        ...
        <b>{year}</b>
      </label><div className="timeline-ticks" aria-label="Demo years">{years.map(item=><button key={item} aria-label={`Year ${item}`} className={year===item?'active':''} onClick={()=>setYear(item)}>{String(item).slice(2)}</button>)}</div></>:<div className="timeline-control"><span>OUTTURN SNAPSHOT</span><b>2025–26</b></div>}
