...
export const nationalData:WallData={
  ...
  domains:moneyOut.filter(flow=>flow.id!=='local').map(flow=>{const meta=domainMeta[flow.id]??realDomainMeta[flow.id];return {...meta,id:flow.id,name:flow.label,short:flow.id==='interest'?'Debt':meta.short,values:snapshot(flow.value),confidence:flow.confidence,dataStatus:flow.dataStatus,evidenceId:`e-${flow.id}`}}),
