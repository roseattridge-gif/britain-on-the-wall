...
export default function App(){
  ...
  const[view,setView]=useState<View>(params().get('view')==='changed'?'changed':params().get('view')==='ask'?'ask':'britain'),[story,setStory]=useState<StoryId>(initialStory),[focus,setFocus]=useState<WallFocus|undefined>(initialMetrics().length?{topicId:initialStory(),periodId:params().get('period')??`${initialYear()}-${String(initialYear()+1).slice(2)}`,metricIds:initialMetrics(),source:'what-changed'}:undefined);
  ...
  const changeView=(next:View)=>{setView(next);const url=new URL(location.href);if(next==='britain')url.searchParams.delete('view');else url.searchParams.set('view',next);url.searchParams.delete('finding');url.searchParams.delete('journey');url.searchParams.delete('step');history.pushState(null,'',url)};
  ...
  const showTarget=(target:FocusTarget)=>{setStory(target.topicId);setYear(Number(target.periodId.slice(0,4)) as RealYear);setFocus({...target,source:'manual'});setView('britain');const url=new URL(location.href);url.searchParams.delete('view');url.searchParams.delete('q');url.searchParams.delete('journey');url.searchParams.delete('step');url.searchParams.set('story',target.topicId);url.searchParams.set('period',target.periodId);if(target.metricIds.length)url.searchParams.set('metric',target.metricIds.join(','));else url.searchParams.delete('metric');url.hash=target.anchor??'';history.pushState(null,'',url)};
