...
const initialView=():View=>params().get('view')==='changed'?'changed':params().get('view')==='ask'?'ask':'britain';
...
export default function App(){
  ...
  const[view,setView]=useState<View>(initialView),[story,setStory]=useState<StoryId>(initialStory),[focus,setFocus]=useState<WallFocus|undefined>(initialFocus);
  ...
  const changeView=(next:View)=>{setView(next);setFocus(undefined);const url=new URL(location.href);if(next==='britain')url.searchParams.delete('view');else url.searchParams.set('view',next);['finding','q','journey','step','metric','focus','value'].forEach(key=>url.searchParams.delete(key));if(next!=='britain')['period','unit','story'].forEach(key=>url.searchParams.delete(key));url.hash='';history.pushState(null,'',url)};
  ...
  useEffect(()=>{const restore=()=>{setView(initialView());setYear(initialYear());setUnit(initialUnit());setStory(initialStory());setFocus(initialFocus());setSelectedFinding(params().get('finding')??undefined);setEvidence(undefined)};addEventListener('popstate',restore);return()=>removeEventListener('popstate',restore)},[]);
